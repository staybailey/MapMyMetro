...
CREATE TABLE agency ( 
  agency_id VARCHAR(255) NOT NULL, agency_url VARCHAR(255), agency_timezone VARCHAR(255), agency_lang VARCHAR(255), agency_phone VARCHAR(255), agency_fare_url VARCHAR(255), PRIMARY KEY (agency_id)
);
...
CREATE TABLE trips (
  route_id INT(11), service_id INT(11), trip_id INT(11) NOT NULL, trip_headsign VARCHAR(255), trip_short_name VARCHAR(255), direction_id INT(11), block_id INT(11), shape_id INT(11), PRIMARY KEY (trip_id), FOREIGN KEY (route_id) REFERENCES routes(route_id)/* FOREIGN KEY (service_id),*/ FOREIGN KEY (block_id) REFERENCES shapes(block_id), /* FOREIGN KEY (shape_id) */
);
